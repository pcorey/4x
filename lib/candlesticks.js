...
Candlesticks.after.insert(function(userId, doc) {
  ...
  trendlines.forEach(function(trendline) {
    ...
    var points = _.union(trendline.candles, doc).map(function(point) {
      return [point.index, point.lowMid];
    });
    var mb = ss.linearRegression(points);
    var line = ss.linearRegressionLine(mb);
    var rr = ss.rSquared(points, line);
    // if (doc.lowMid < line(doc.index)) {
    if (rr < 0.9 && doc.lowMid < line(doc.index)) {
      console.log("Break!");
